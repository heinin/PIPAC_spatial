...
gse_kegg <- enrichKEGG(setdiff(gse_input$entrez, c(NA)),
                       keyType = "kegg",
                       #minGSSize = 3, 
                       #maxGSSize = 800, 
                       pvalueCutoff = 0.1, 
...
top.table$group <- ifelse(top.table$gene_name %in% secretory_markers$Gene.name, "Secretory",
                          ifelse(top.table$gene_name %in% surface_markers$Gene.name, "Surface", "Other"))
...
#top.table_secr %>% filter(gene_name == "TP53I3")

metadata_expression$Treatment <- factor(metadata_expression$Treatment,
                                        levels = c("V", "MMC"))

metadata_expression %>%
  ggplot(aes(x = Treatment, y = ENSG00000175063, fill = Treatment)) +
  geom_violin(alpha = 0.5) +
  geom_point(position = position_jitter(seed = 1, width = 0.2)) +
  ylab("UBE2C expression (FPKM)") +
  xlab("Treatment") +
  theme_classic() +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
