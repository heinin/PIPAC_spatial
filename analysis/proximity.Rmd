...
setdiff(proximity_compiled_tumor$anchor_cell_ct, seurat_data$Celltype_Final)
setdiff(seurat_data$Celltype_Final, proximity_compiled_tumor$anchor_cell_ct)
...
enrichment_scores_compiled_tumor <- enrichment_scores_compiled_tumor %>%
  filter(prox_to %in% keeps$Var1 & prox_ct %in% keeps$Var1)

proximity_compiled_tumor <- proximity_compiled_tumor %>%
  filter(anchor_cell_ct %in% keeps$Var1 & prox_cell_ct %in% keeps$Var1)
...
summaries <- proximity_compiled_tumor %>%
  ...
  filter(Tissue == "Tumor",
         ...
         prox_cell_ct %in% setdiff(unique(seurat_data$Celltype_Updated), c("F1", "F2", "F3", "F4", "F5"))) %>%
...
```{r, message = F, warning = F, fig.width = 20, fig.height = 20, eval = F}
...
enrichment_scores_compiled <- read.table("/home/hnatri/PIPAC_spatial/enrichment_tumor_pretreatment_scores_compiled.tsv",
                                         header = T)
...
enrichment_scores_compiled <- enrichment_scores_compiled %>%
  filter(prox_to %in% keeps$Var1 & prox_ct %in% keeps$Var1)

proximity_compiled <- proximity_compiled %>%
  filter(anchor_cell_ct %in% keeps$Var1 & prox_cell_ct %in% keeps$Var1)
...
hp <- Heatmap(ct_or, name = 'log(OR)',
              ...
              width = ncol(ct_or)*unit(8, "mm"),
              height = nrow(ct_or)*unit(8, "mm"))
...
post_enrichment_scores_compiled <- read.table("/home/hnatri/PIPAC_spatial/enrichment_tumor_posttreatment_scores_compiled.tsv",
                                         header = T)
post_proximity_compiled <- read.table("/home/hnatri/PIPAC_spatial/tumor_posttreatment_proximity_compiled.tsv",
                                 header = T)

post_enrichment_scores_compiled <- post_enrichment_scores_compiled %>%
  filter(prox_to %in% keeps$Var1 & prox_ct %in% keeps$Var1)

post_proximity_compiled <- post_proximity_compiled %>%
  filter(anchor_cell_ct %in% keeps$Var1 & prox_cell_ct %in% keeps$Var1)
