---
title: "Xenium data preprocessing, whole cell transcripts"
author: "heinin"
date: "2025-07-01"
output:
...
for (tma in tma_ids) {
  message(paste("Getting cell counts for sample", tma))
...
#saveRDS(nuc_obj_list, "/scratch/hnatri/PIPAC/nuc_obj_list.rds")
...
nuc_obj_list <- readRDS("/scratch/hnatri/PIPAC/nuc_obj_list.rds")

# Merge objects (cannot do spatial DimPlots for this)
#merged_spatial_unfiltered <- nuc_obj_list[[1]]
merged_spatial_unfiltered <- merge(x = nuc_obj_list[[1]], y = nuc_obj_list[2:length(nuc_obj_list)])
...
# Merge objects
#merged_spatial_unfiltered <- nuc_obj_list[[1]]
cell_merged <- merge(x = obj_list[[1]], y = obj_list[2:length(obj_list)])
...
head(merged_spatial_unfiltered@assays$RNA$counts)
head(merged_spatial_unfiltered@assays$cell_RNA$counts)
...
BetterVlnPlot(merged_spatial_unfiltered,
              features = "nCount_cell_RNA", group_by = "TMA")
BetterVlnPlot(merged_spatial_unfiltered,
              features = "nFeature_cell_RNA", group_by = "TMA")
...
# All transcripts
# nCount_RNA vs. nFeature_RNA
smoothScatter(log(merged_spatial_unfiltered$nCount_cell_RNA),
              log(merged_spatial_unfiltered$nFeature_cell_RNA),
              cex = 0.5, pch = 16)
abline(v = log(15), h = log(15), lty = "dashed", col = "black")
text(0.3, 4.6, col = "black", adj = c(0, -.1),
     "nCount_cell_RNA >= 15 & nFeature_cell_RNA >= 15")

smoothScatter(merged_spatial_unfiltered$nCount_cell_RNA,
              merged_spatial_unfiltered$nFeature_cell_RNA,
              cex = 0.5, pch = 16)
abline(v = log(15), h = log(15), lty = "dashed", col = "black")
text(0.3, 4.6, col = "black", adj = c(0, -.1),
     "nCount_cell_RNA >= 15 & nFeature_cell_RNA >= 15")
...
merged_spatial <- subset(merged_spatial_unfiltered,
                          subset = nCount_cell_RNA >= 15 & nFeature_cell_RNA >= 15 &
                            #percent.blank <= 5 & 
...
head(merged_spatial@assays$RNA$counts)
head(merged_spatial@assays$cell_RNA$counts)

# To build on command line, run Rscript -e "rmarkdown::render('Xenium_preprocess_wholecell.Rmd')"
