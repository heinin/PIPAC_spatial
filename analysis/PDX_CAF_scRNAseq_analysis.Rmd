...
main_annotations <- read_sheet(metadata, sheet = "Final main annotations")
...
# Renaming Lymphoid3_GZMB
seurat_integrated$Annotation <- gsub("Lymphoid3_GZMB", "Lymphoid4_GZMB", seurat_integrated$Annotation)

Idents(seurat_integrated) <- seurat_integrated$Annotation
seurat_integrated <- FindSubCluster(seurat_integrated,
  cluster = "CAF6",
  graph.name = "harmony_snn",
  subcluster.name = "Annotation_subcluster",
  resolution = 0.5,
  algorithm = 1)

table(seurat_integrated$Annotation_subcluster)

seurat_integrated$Annotation_subcluster <- ifelse(seurat_integrated$Annotation_subcluster %in% c("CAF6_0", "CAF6_1", "CAF6_2"), "Myeloid6",
                                                  ifelse(seurat_integrated$Annotation_subcluster == "CAF6_3", "CAF6", seurat_integrated$Annotation_subcluster))

seurat_integrated$Annotation_subcluster <- factor(seurat_integrated$Annotation_subcluster,
                                            levels = sort(unique(seurat_integrated$Annotation_subcluster)))

seurat_integrated$Annotation <- seurat_integrated$Annotation_subcluster
...
seurat_integrated$Updated_Annotation <- mapvalues(x = seurat_integrated$Annotation,
                                                  from = main_annotations$Celltype,
                                                  to = main_annotations$`Descriptive name/Updated celltype`)

saveRDS(seurat_integrated, "/tgen_labs/banovich/PIPAC/CAF_scRNAseq/PDX_CAF_Seurat_annotated.rds")

```

## Plotting annotations and marker expression

```{r, message = F, warning = F, fig.width = 9, fig.height = 16}
...
# For plotting
top_cluster_markers <- markers %>% group_by(group) %>% slice_max(order_by = auc, n = 10)

```

```{r, message = F, warning = F, fig.width = 9, fig.height = 24}

create_dotplot_heatmap(seurat_object = seurat_integrated,
                       plot_features = unique(top_cluster_markers$feature),
                       group_var = "Updated_Annotation",
                       group_colors = ct_col,
                       column_title = "",
                       row_km = 5,
...
```{r, message = F, warning = F, fig.width = 12, fig.height = 6, eval = T}
...
```{r, message = F, warning = F, fig.width = 10, fig.height = 50, eval = T}
...
```{r, message = F, warning = F, fig.width = 10, fig.height = 22, eval = T}
...
```{r, message = F, warning = F, fig.width = 10, fig.height = 44, eval = T}
...
mouse_immune <- c("Ptprc", "Cd3e", "Cd3g", "Cd247", "Cd4", "Cd8a", "Cd8b1", 
                  ...
                  "Pdcd1")
mouse_immune <- unique(mouse_immune)
...
```{r, message = F, warning = F, fig.width = 10, fig.height = 17, eval = T}
