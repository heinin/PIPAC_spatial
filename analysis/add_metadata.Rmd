...
#setdiff(colnames(nucl_seurat_data), colnames(seurat_data))
#setdiff(colnames(seurat_data), colnames(nucl_seurat_data))
#intersect(colnames(seurat_data), colnames(nucl_seurat_data))

seurat_data$Annotation <- mapvalues(colnames(seurat_data),
                                    from = colnames(nucl_seurat_data),
                                    to = nucl_seurat_data$Annotation)
seurat_data$Annotation[-which(colnames(seurat_data) %in% colnames(nucl_seurat_data))] <- NA
...
saveRDS(seurat_data, "/tgen_labs/banovich/PIPAC/Seurat/cell_merged_spatial_filtered_splitsamples_clustered_NN30_PC50_Seurat_metadata.rds")
