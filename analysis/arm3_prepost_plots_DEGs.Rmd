...
wholetumor_DEGs <- FindMarkers(arm3_tumor,
                               ident.1 = Treated,
                               ident.2 = Baseline,
                               assay = "RNA",
                               verbose = F)
wholetumor_DEGs$feature <- rownames(wholetumor_DEGs)

wholetumor_DEGs_sig <- wholetumor_DEGs %>%
  filter(p_val_adj < 0.01,
         abs(avg_log2FC) > 1.5)

dim(wholetumor_DEGs_sig)
unique(wholetumor_DEGs_sig$feature)
wholetumor_DEGs_sig
