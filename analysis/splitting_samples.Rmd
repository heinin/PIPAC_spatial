...
head(seurat_data@assays$RNA$counts)
head(seurat_data@assays$cell_RNA$counts)
...
# Cell stats
cellstat_path <- "/tgen_labs/banovich/PIPAC/Cell_stats/"
...
names(merged_spatial@assays) <- c("nucleus_RNA", "RNA")
DefaultAssay(merged_spatial) <- "nucleus_RNA"
merged_spatial <- JoinLayers(merged_spatial)
DefaultAssay(merged_spatial) <- "RNA"

#head(merged_spatial@assays$nucleus_RNA$counts)
#head(merged_spatial@assays$RNA$counts)

#saveRDS(merged_spatial, "/scratch/hnatri/PIPAC/cell_merged_spatial_filtered_splitsamples.rds")
