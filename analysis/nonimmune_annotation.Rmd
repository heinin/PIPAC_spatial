...
seurat_data <- readRDS("/scratch/hnatri/PIPAC/nonimmune_clustered_NC50_NN20_PC20_Seurat.rds")
...
first_pass_annot <- read_sheet(metadata, sheet = "Tumor annotations 1st pass")
...
write.table(output_cluster_markers, "/home/hnatri/PIPAC_spatial/nonimmune_cluster_marker_annotations.tsv",
            quote = F, row.names = F, sep = "\t")

```

### Subsetting remaining immune cells and adding nonimmune cells from the annotated immune object back
...
#saveRDS(seurat_data, "/scratch/hnatri/PIPAC/nonimmune_subset.rds")

#immune_subset <- subset(seurat_data, subset = annot1 %in% c("Plasma", "DC1"))
nonimmune_subset <- subset(seurat_data, subset = annot1 %in% c("Plasma", "DC1"), invert = T)

immune <- readRDS("/scratch/hnatri/PIPAC/immune_subset.rds")
nonimmune_from_immune <- subset(immune, annot1 == "NonImmune")

#saveRDS(nonimmune_subset, "/scratch/hnatri/PIPAC/nonimmune_subset_for_2ndpass.rds")
#saveRDS(immune_subset, "/scratch/hnatri/PIPAC/immune_subset_from_nonimmune.rds")

nonimmune_complete_subset <- merge(nonimmune_subset, nonimmune_from_immune)

unique(nonimmune_complete_subset$annot1)

saveRDS(nonimmune_complete_subset, "/scratch/hnatri/PIPAC/nonimmune_complete_subset.rds")
